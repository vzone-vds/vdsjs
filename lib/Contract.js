...
class Contract {
     ...
     * @return parse data
     */
    async callFunc(callMethod, callArgs = [], opts = {}) {
        const methodABI = this.methodMap.findMethod(callMethod, callArgs);
        if (!methodABI) {
            throw new Error(`Unknown method to call: ${callMethod}`);
        }
        const r = await this.rpc.callContractFunc(Object.assign({ address: this.address, method: callMethod, args: callArgs }, opts));
        return r;
    }
    /**
