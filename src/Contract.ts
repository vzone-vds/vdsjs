...
export class Contract {
     ...
     */
    constructor(private rpc: VdsRPC, public info: IContractInfo, opts: IContractInitOptions = {}) {
    ...
    public async callFunc(
        callMethod: string, callArgs: any[] = [],
        opts: IContractCallRequestOptions = {}):
        Promise<any> {
        const methodABI =  this.methodMap.findMethod(callMethod, callArgs)
        if (!methodABI) {
            throw new Error(`Unknown method to call: ${callMethod}`)
        }
        const r = await this.rpc.callContractFunc({
            address: this.address,
            method: callMethod,
            args: callArgs,
            ...opts
